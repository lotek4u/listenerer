    ...
    public sealed class SerializableAudioMediaBuffer : IDisposable
    {
        ...
        public class SerializableUnmixedAudioBuffer
        {
            ...
            public SerializableUnmixedAudioBuffer(UnmixedAudioBuffer buffer, IParticipant participant)
            {
                ...
                // Listenerer: PSTN callers have no AAD user and often no display name.
                // Fall back to the active speaker id so their audio is never dropped
                // and the WAV always has a deterministic name.
                if (string.IsNullOrWhiteSpace(AdId))
                {
                    AdId = $"speaker-{buffer.ActiveSpeakerId}";
                }
