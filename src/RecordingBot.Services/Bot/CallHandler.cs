...
namespace RecordingBot.Services.Bot
{
    public class CallHandler : HeartbeatHandler
    {
        ...
        // Listenerer: metadata captured over the call lifetime, written to metadata.json on termination.
        private DateTimeOffset? _establishedAt;
        private bool _metadataWritten = false;
        private readonly Dictionary<string, string> _participantIdentities = new();
        ...
        private async void CallOnUpdated(ICall sender, ResourceEventArgs<Call> e)
        {
            ...
            if ((e.OldResource.State == CallState.Established) && (e.NewResource.State == CallState.Terminated))
            {
                ...
                // Listenerer: drop metadata.json next to the WAVs for the scoring app.
                WriteCallMetadata();
            }
        }
        ...
        private void UpdateParticipants(ICollection<IParticipant> eventArgs, bool added = true)
        {
            foreach (var participant in eventArgs)
            {
                ...
                {
                    // Listenerer: remember adId -> display name so metadata.json can label each WAV.
                    if (added && !string.IsNullOrWhiteSpace(participantDetails.Id))
                    {
                        _participantIdentities[participantDetails.Id] = participantDetails.DisplayName;
                    }
                ...
                {
                    // Listenerer: PSTN/phone participants have no AAD user — record their
                    // phone identity so metadata.json can label the fallback-named WAVs.
                    if (added)
                    {
                        foreach (var i in participant.Resource.Info.Identity.AdditionalData)
                        {
                            if (i.Key != "applicationInstance" && i.Value is Identity pid && !string.IsNullOrWhiteSpace(pid.Id))
                            {
                                _participantIdentities[pid.Id] = !string.IsNullOrWhiteSpace(pid.DisplayName)
                                    ? pid.DisplayName
                                    : $"PSTN {pid.Id}";
                                break;
                            }
        ...
        // Listenerer: write metadata.json into the same folder as the call's WAVs.
        private void WriteCallMetadata()
        {
            if (_metadataWritten)
            {
                return;
            ...
            try
            {
                var folder = RecordingPaths.GetCallFolder(_settings, Call.Id);
                ...
                var metadata = new
                {
                    ...
                    observedParticipantId = Call.Resource?.IncomingContext?.ObservedParticipantId,
                    startTimeUtc = _establishedAt,
                    endTimeUtc = endedAt,
                    durationSeconds = _establishedAt.HasValue
                        ? (endedAt - _establishedAt.Value).TotalSeconds
                        : (double?)null,
                    participants = _participantIdentities
                        .Select(kv => new { adId = kv.Key, displayName = kv.Value })
                        .ToArray(),
                    recordingFolder = folder,
                    note = "Each participant WAV is named by adId: AAD object id for Teams users, phone number for PSTN callers, or speaker-<id> when no identity was available. When the call has a PSTN leg, single-sided WAVs carry the caller's number as a suffix, e.g. <adId>-[13098214236].wav — match files by adId prefix. 'all.wav' is the mixed stream and is never suffixed."
                };
                ...
                _eventPublisher.Publish("CallMetadataWritten", $"Call.Id: {Call.Id} -> {folder}");
            }
            catch (Exception ex)
            {
                GraphLogger.Error(ex, "Listenerer: failed to write metadata.json");
            }
        }
