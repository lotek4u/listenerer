...
namespace RecordingBot.Services.ServiceSetup
{
    public partial class AzureSettings : IAzureSettings
    {
        ...
        // Listenerer: configurable recording sink. Local dir or UNC share (e.g. D:\recordings or \\nas\recordings).
        // Recordings land in <RecordingRootFolder>/<callId>/. Defaults to the OS temp folder when unset.
        public string RecordingRootFolder { get; set; }
        ...
        public void Initialize()
        {
            ...
            // Listenerer: default the recording sink to the temp folder if not configured.
            if (string.IsNullOrWhiteSpace(RecordingRootFolder))
            {
                RecordingRootFolder = Path.Combine(Path.GetTempPath(), BotConstants.DEFAULT_OUTPUT_FOLDER);
            }
